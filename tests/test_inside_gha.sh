...
function test_docker_startup {
    ...
    logfile=$(wait_for_output 120 run_inside_backfill_container find /pilot -name StartLog)
    if [[ -z $logfile ]]; then
        docker ps -a
...
function test_singularity_startup {
    ...
    logfile=$(wait_for_output 120 run_inside_singularity_backfill find /pilot -name StartLog)
    if [[ -z $logfile ]]; then
...
case "$CVMFS_INSTALL" in
    bindmount)
        ...
        ;;
    cvmfsexec)
        DOCKER_EXTRA_ARGS=(--privileged -e CVMFSEXEC_REPOS='oasis.opensciencegrid.org singularity.opensciencegrid.org')
        ;;
esac
